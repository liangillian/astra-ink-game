...
function draw(){
 ...
 const ok=confirm(`本次抽卡將消耗 ${CARD_POOL_INFO.costSingle} 星鑽。\n目前持有：${state.gems}\n抽卡後剩餘：${state.gems-CARD_POOL_INFO.costSingle}\n\n確定要抽嗎？`);
 if(!ok)return;
 if(!spendGems(state,CARD_POOL_INFO.costSingle))return;
 state.pulls++;
 addQuestProgress(state,"gacha",1);
